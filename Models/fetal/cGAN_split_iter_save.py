...
##################### Training Procedure #######################################
    ...
    def train(self, fold_no, epochs, batch_size=1, sample_interval=50, iter=False, folds=1):
        ...
        if sys.version_info[0] < 3:
            execfile("/homes/wt814/IndividualProject/code/BatchGeneratorClass/BatchGenerator_v2_config_Full.cfg", confTrain)
        else:
            exec(open("/homes/wt814/IndividualProject/code/BatchGeneratorClass/BatchGenerator_v2_config_Full.cfg").read(),confTrain)
    ...
    def sample_images(self, epoch, x, x1, x2, x3, affine, fold):
        ...
        nib.save(save_img, 'niftiImage/splitGenIterSave/Test/%d/%d.nii.gz' %(fold+1,epoch))
    ...
    def sample_all(self, x, x1, x2, x3, affine, fold):
        for i in range(len(x)):
            ...
            nib.save(save_img, 'niftiImage/splitGenIterSave/Sample/%d.nii.gz' %(i))

        self.generator.save('cGAN_generator.h5')
        self.combined.save('cGAN_full.h5')
        self.discriminator.save('cGAN_discriminator.h5')

if __name__ == '__main__':

    cgan = CGAN()
    #cgan.train(fold_no=1, epochs=3000, batch_size=2, sample_interval=100)
